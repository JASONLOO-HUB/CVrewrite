...
- 仓库扫描脚本：运行 [scripts/scan_repo_context.py](scripts/scan_repo_context.py)
...
第一步必须运行仓库扫描脚本，对代码仓库做上下文索引：

```bash
python3 scripts/scan_repo_context.py /path/to/repo
```

脚本只负责输出高价值上下文索引，不负责替你判断原始意图。

如果 `JD` 或 `原简历` 文件本身就放在仓库里，扫描时要显式排除，避免污染项目上下文：
